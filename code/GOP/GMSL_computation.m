...
path_C2Data = '/Volumes/scratch/general/cryosat/validation_data/';
...
for i=1:nmonths
    ...
    kj = k0:kf;
    for j=1:length(kj)
        data = load([path_C2Data files(kj(j),:)]);
        xtemp = data.x_ssha;
        ytemp = data.y_ssha;
        vtemp = data.ssha;
        ttemp = data.t_ssha;
        otemp = data.orbit_ssha;
        x{i} = [x{i} xtemp];
        y{i} = [y{i} ytemp];
        v{i} = [v{i} vtemp];
        t{i} = [t{i} ttemp];
...
hl = legend([h1,h2],'Colorado University','CryoSat-2','location','northWest');
set(gca,'FontSize',16)
ylabel('GMSL (cm)')
ylim([5.0 max(gmsl)+0.9])
