...
in_dir = ['/noc/mpoc/cryo/cryosat/' data_type '/'];
path1 = '/noc/mpoc/cryo/cryosat/daily_stats/';
path2 = '/noc/mpoc/cryo/cryosat/daily_data/';
...
ndays = length(recDate) + sum(kempty); % modified on 28 Sep 2016
...
if any(kempty)
    datesn = date0:datef;
    laten = [laten 1000.*ones(1,sum(kempty)*2)]; %set latency to a large value since data were never delivered
    tii = repmat(datesn(kempty),2,1);
    tlaten = [tlaten tii(:)'];
    [tlaten,ix] = sort(tlaten);
    laten = laten(ix);
end
% --------------------------------------------------------------------------
...
for j=1:length(params) % loop over all parameters
    ...
    if j == 5 && strcmp(data_type,'SIR_IOP_L2')
        ...
        if any(kempty)
            pi1 = zeros(1,ndays);
            pi2 = zeros(1,ndays);
            pi1(~kempty) = percRecxTheor;
            pi2(~kempty) = percRecxTheorPos;
            percRecxTheor = pi1;
            percRecxTheorPos = pi2;
        end
        % --------------------------------------------------------
    ...
    if j == 5 % mispointing
        ...
        if any(kempty)
            datesn = date0:datef;
            x = [x NaN(1,sum(kempty)*2)];
            tii = repmat(datesn(kempty),2,1);
            recDatex = [recDatex tii(:)'];
        end
        % --------------------------------------------------------------------------
        ...
        xp = 1:ndays; % modified on 28 Sep 2016
        boxWhiskers(x*scale(j),recDatex,units{j},dates,rgb3,'black',ptile)
        hh = findobj(gcf,'tag','Outliers');
        set(hh,'Visible','off')
        
        qi1 = data.(['mean_' vari{j}]).*scale(j); % added on 28 Sep 2016
        qi2 = data.(['std_' vari{j}]).*scale(j); % added on 28 Sep 2016
        % --------------------- modified on 28 Sep 2016 ----------------------------------
        if any(kempty)
            qii = NaN(1,length(xp));
            qii(~kempty) = qi1;
        ...
        % --------------------------------------------------------------------------------
        hs = shadedErrorBar(xp,qi1, qi2,{'-','color',rgb2,'LineWidth',1.5,'markerfacecolor',rgb2});
    ...
    if(j < 5) % exclude mispointing
        ...
        %pr = zeros(2,length(recDatexU));
        pr = NaN(2,length(recDatexU)); % modified on 28 Sep 2016
        ptile = [5 95];
        for f=1:length(recDatexU)
            kj = recDatex == recDatexU(f);
            pr(:,f) = prctile(x(kj)*scale(j),ptile);
        ...
        if j ~= 1
            ...
            qi1 = data.(['meanNOC_' vari{j}]).*scale(j); % added on 28 Sep 2016
            qi2 = data.(['stdNOC_' vari{j}]).*scale(j); % added on 28 Sep 2016
            % --------------------- modified on 28 Sep 2016 ----------------------------------
            if any(kempty)
                qii = NaN(1,length(xp));
                qii(~kempty) = qi1;
            ...
            if(strcmp(data_type,'SIR_IOP_L2') || strcmp(data_type,'SIR_GOP_2'))
                ...
                % --------------------------------------------------------------------------------
                hs1 = errorbar(xp,qi1, qi2,'o','color',rgb2,'LineWidth',0.7,'MarkerSize',4,'MarkerFaceColor',rgb2);
            else
                qi1 = data.(['meanNOC_' vari{j}]).*scale(j);
                qi2 =  data.(['stdNOC_' vari{j}]).*scale(j);
                % --------------------- modified on 28 Sep 2016 ----------------------------------
                if any(kempty)
                    qii = NaN(1,length(xp));
                    qii(~kempty) = qi1;
                ...
                errorbar(xp,qi1, qi2,'o','color',rgb2,'LineWidth',0.7,'MarkerSize',4,'MarkerFaceColor',rgb2);
            end
            ...
            if(strcmp(data_type,'SIR_IOP_L2') || strcmp(data_type,'SIR_GOP_2'))
                ...
                % --------------------------------------------------------------------------------
                hold on
                hs2 = shadedErrorBar(xp,qi1, qi2,{'-o','color',rgb1,'LineWidth',1.5,'markerfacecolor',rgb1,'MarkerSize',3});
        ...
        if j == 1
            ...
            % --------------------------------------------------------------------------------
            errorbar((1:ndays)',qi1,qi2,'o','color',rgb2,'LineWidth',0.7,'MarkerSize',4,'MarkerFaceColor',rgb2);
            ylabel(units{j},'FontSize',10)
