...
def genReportFun():
    print "yes I am in TDS mode"
    template_odt = 'CryOcean-QCV_MonthRepTDS'
    #pathOut = '/scratch/general/cryosat/newReports_monthly/' changed 26/10/16
    pathOut = '/noc/mpoc/cryo/TDS_testout/report/'
    path = '/noc/users/cryo/QCV_Cryo2/code/gen_monthly_report/'
    image_dir = path + 'figures/TDS/'
    odt_file_ext = '.odt'
    ...
    for n,fid in enumerate(fidAll):
        ...
        fld['median_latency' + prod[n]] = 0
        fld['latencyLow' + prod[n]] = 0
        fld['latencyUp' + prod[n]] = 0
        ...
        Report_Date = "August 2013 (TDS)"   #Report_Date.replace(" ","/")
        print Report_Date
        ...
        ii = fid.readline()
        ii = ii.split()
        fld['total_nrecord' + prod[n]] = 0
        fld['total_precord' + prod[n]] = 0
        ii = fid.readline()
        ii = ii.split()
        fld['ocean_nrecord' + prod[n]] = 0
        fld['ocean_precord' + prod[n]] = 0
    ...
    # Generate the Report
    Report_fn = template_odt + "_" + "August_2013_TDS"
