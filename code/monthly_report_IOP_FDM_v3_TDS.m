function monthly_report_IOP_FDM_v3_TDS(data_type,report_date)


updated to use monthly_report_IOP_FDM_v3_nodisplay_TDSv2
addpath(genpath('/noc/users/cryo/QCV_Cryo2/code/'))
% report_date = 'Mar-2015'; %mmm-yyyy (month-year)
...
for i=1:size(files,1)
    date_of_interest = t_window(i,:) ;
    if ~exist([path2 files(i,:)],'file') && ~exist([path1 files_stat(i,:)],'file')
%         read_cryo2DatavC([in_dir date_of_interest(1:4) '/' date_of_interest(5:6) '/'],...
%             path2,date_of_interest,date_of_interest);
        read_cryo2DatavC(in_dir ,path2,date_of_interest,date_of_interest);

    end
    if ~exist([path2 files(i,:)],'file') && ~exist([path1 files_stat(i,:)],'file')
        dailyEmptyStats(date_of_interest,data_type,path1); % create empty structure if no data
        kempty(i) = true;
    end
end
%
...
data = struct([]);
corrections = struct([]);
warning('changed just below from daily_stats_v6 to daily_stats_v6_noSARin')
for i=1:size(files_stat,1)
    if ~exist([path1 files_stat(i,:)],'file')
        daily_stats_v6_noSARin(t_window(i,:),data_type2,path2,in_dir,path1, ...
            DTU10MSS,gshhs_i,A);
    end
    ...
    if strcmp(data_type2,'SIR_GOP_L2')
...
% % % % % % % % % % % % % % % % % %     perc_3d(f) = sum(laten(kj) <= thr1)/sum(kj)*100;
...
for j=1:length(params) % loop over all parameters
    ...
    if j==1
        ...
        if(strcmp(data_type,'SIR_IOP_L2') || strcmp(data_type,'SIR_GOP_L2'))
            ...
            % % %             y_noise2DHist_outLRM = y_noise2DHist_outLRM(fout);
        else
            fin = f1(f3 & f4 & f5);
        end
        %         y_noise2DHist_inLRM = y_noise2DHist_inLRM(fin);
    ...
    if any(~kempty)
        ...
        for i=1:size(files,1)
            if ~exist([path1 files(i,:)],'file')
                ...
                error('possibly need no SARIn version need to check if reach this point')
...
if strcmp(data_type,'SIR_GOP_L2')
    ...
    for j=1:length(params) % loop over all corrections
        ...
        pPos = [13.9 9.4];
        pos = [10.5 9];
        plot_map_nodisplay_monthly(cnt,lonx,latx,x.*scale(j),cmax(j), ...
            cmin(j),cytick(j),units{j},titl,pPos,pos);
        ht = plotTable([],strCol,dat_tab,[xposTab(j) 0.48],8,1);
        set(ht(1,1:end-2),'FontWeight','bold','BackGroundColor', ...
            [206/255 235/255 251/255])
