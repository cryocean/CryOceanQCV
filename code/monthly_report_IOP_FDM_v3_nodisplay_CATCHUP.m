function monthly_report_IOP_FDM_v3_nodisplay_CATCHUP(data_type,report_date)
...
dir_out = '/noc/users/cryo/QCV_Cryo2/code/gen_monthly_report/figures/catchup/';
...
for i=1:size(files,1)
    ...
    if exist([path2  files(i,12:15) '/' files(i,16:17)  '/'  files(i,:)],'file') ~= 2
    ...
    if exist([path2  files(i,12:15) '/' files(i,16:17)  '/'  files(i,:)],'file') ~= 2 && ...
            exist([path1  files(i,12:15) '/' files(i,16:17)  '/'  files_stat(i,:)],'file') ~= 2
...
for i=1:size(files_stat,1)
    if ~exist([path1 files_stat(i,18:21) '/' ...
            files_stat(i,22:23) '/' files_stat(i,:)],'file')
        daily_stats_v6_FOR_CATCHUP(t_window(i,:),data_type,path2,in_dir,path1, ...
            DTU10MSS,gshhs_i,A);
    end
    data = [data load([path1 files_stat(i,18:21) '/' ...
        files_stat(i,22:23) '/' files_stat(i,:)])]; %#ok %this only works if there is no struct in struct
    tmp = load([path1 files_stat(i,18:21) '/' ...
        files_stat(i,22:23) '/' files_stat(i,:)]);
...
for j=1:length(params) % loop over all parameters
    ...
    if j < 4
        ...
        pPos = [13.9 9.2];
        pos = [10.5 9];
        % extra code to remove NaNs
        % have saved data as chris_temp.mat
