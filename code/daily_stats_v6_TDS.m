function daily_stats_v6_TDS(day_date,data_type,path1,path2,path3,mss,gshhs,ground)


% copy of daily_stats_v6, hopefully with all changes marked up by
%  CHANGED FOR TDS
...
outSARin_hi = isLRM(data.hi_lon,data.hi_lat,SARin_mask);
...
clear ix
% repeat for 20 Hz data
tutc_hi = data.hi_utctime;
[~,ix] = unique(tutc_hi,'stable'); % there are often repeated values
kt_hi = false(size(tutc_hi));
kt_hi(ix) = true;
if strcmp(data_type,'SIR_FDM_L2')
    kt_hi = and(kt_hi,~outLRM_hi); % we reject points for FDM in non-FDM regions
else
    kt_hi = and(kt_hi,outSARin_hi);
end
hi_utc = data.hi_utctime(kt_hi);
clear ix
...
lon = data.lon(kt);
lat = data.lat(kt);
hi_lon = data.hi_lon(:,kt_hi); % high rate lon
hi_lat = data.hi_lat(:,kt_hi);
surface_type = data.flag_surface_type(kt);
flag_block = data.flag_mcd_block_degraded(:,kt); %records should not be used if 1
% 20 Hz
% % % % % % % % surface_type = data.flag_surface_type(kt);
% % % % % % % % flag_block = data.flag_mcd_block_degraded(:,kt_); %records should not be used if 1
...
% % %     flag_swh_av = data.flag_swh_av(:,kt);
    hi_swh = data.hi_swh(:,kt_hi);
end

% DEPTH
% CHANGED FOR TDS
depth = data.swh(kt) + NaN;
% depth = data.h_dem(kt);
...
hi_h = data.hi_h(:,kt_hi);
hi_range = data.hi_range(:,kt_hi);
...
hi_sigma0 = data.hi_sigma0(:,kt_hi);
...
for i=1:5
    ...
    if i < 4
        ...
        %         x(nflg < 10) = NaN;  CHANGED FOR TDS AS THIS SEEMS PROBLEMATIC
        %         hi_x(:,nflg < 10) = NaN;CHANGED FOR TDS AS THIS SEEMS PROBLEMATIC
        params{i} = x;
        hi_params{i} = hi_x;
    end
    x = params{i};
    ...
    if i == 1
        ...
        for j=1:length(gu)
            kg = orbits == gu(j) & nan1{i}; % valid measurement for given orbit
            %             warning('changed orbit bias threshold from 0.9 to ')
            %             large_orbit_bias: this warning refers to individual orbits and
            %             is issued whenever there are more than 100 flag-valid records
            %             with a SSH anomaly larger than ±90 cm.
    ...
    if i < 5
        ...
        if i == 1 % For SLA only
            ...
            %            CHANGED FOR TDS editSLAstd = nanstd(hi_x) <= thrstd(i);
            ...
            %            CHANGED FOR TDS             editSigma0std = nanstd(hi_x2) <= thrstd(3);
            editSigma0std = editSigma0;
        ...
        elseif i ~= 4
            if i == 2
                ...
                % CHANGED FOR TDS editSWHstd = nanstd(hi_x) <= thrstd(i);
                editSWHstd = editSWH;
            end
     % CHANGED FOR TDS
...
% changed for TDS   dataStat.percEditSigma0stdSLA = sum(~editSigma0std(nan1{1} & noPolar))/n1*100;
dataStat.percEditSigma0stdSLA = NaN;
dataStat.percEditSigma0 = sum(~editSigma0(nan1{3} & noPolar))/n3*100;

% CHANGED FOR TDS dataStat.percEditSigma0std = sum(~editSigma0std(nan1{3} & noPolar))/n3*100;
dataStat.percEditSigma0std = NaN;
dataStat.percEditSWH = sum(~editSWH(nan1{2} & noPolar))/n2*100;

% CHANGED FOR TDS dataStat.percEditSWHstd = sum(~editSWHstd(nan1{2} & noPolar))/n2*100;
dataStat.percEditSWHstd = NaN;
...
for j=1:3
    ...
    %     else
    dataStat.(['absDiff_' fields{j}]) = NaN(20,length(nan_x));
    %     end
end
%end
