...
def getMissingDate():
    ...
    fileNew = [x for x in fileNew if (x[0] == '2' and x[5] == 'w')]
...
def getSwhWspData():
    ...
    path_out = path_in + year + "_wsp_swh/"
    ...
    path2Data = 'http://www.ndbc.noaa.gov/data/historical/stdmet/'
    for ID in stId:
        fln = ID + 'h' + year + '.txt.gz'
        URL3 = path2Data + fln
        ...
        try:
         ...
         #   print "No data for filename2 " + fln 
            fileFid.close()
            continue
        ...
        if ytmp.ndim > 1:
            for mi in monthi:
                ...
                with open(path_out + month[mi] + "/" + ID + '.txt','w') as fid:
                    ...
                    np.savetxt(fid,yi,fmt='%d %02d %02d %02d %d %d %.1f %.1f %.2f %.2f %.2f \
                     %d %.1f %.1f %.1f %.1f %.1f %.2f') #write data in same file
    ...
    if (year == year0) and (monthi == monthi0): 
        for mi in monthi:
            path2Data = 'http://www.ndbc.noaa.gov/data/stdmet/' + month[mi] + '/'
            try:
            ...
            for ID in stId:
                ...
                try:
                    ...
                    fileFid.close()
                    #print "(SWH) Got data for station " + ID + "  " + fln
                except IOError:
                    #print "(SWH) No data for station " + ID + "  " + fln
                    fileFid.close()
                    continue
